...
var animate_time = 3000; //steps before the animation stops 
...
function animateWorld() {
	for (var j = 0; j < ffRate; j++) {
		...
		if (!clicking && timer >= animate_time) {
			endVis();
			break;
		}
		if (clickGravity) {
...
//LOAD A CLIP AND ANIMATE IT
function loadClipNoAnimate(number) {
	$.ajax({
		url: 'data/' + number + '.json',
		dataType: 'json',
		success: function(response) {
...
function generateBrickPositions() {
	...
	for (var i = 0; i < bricks_num; i++) {
		...
		if (loading_data) {
			...
			var brick_y = (i === brick_special) ? 3.5 : Math.random() * (maxNum - minNum) + minNum;
...
function createBox(width, height, x, y, angle, type, density, friction, restitution, userData, img) {
	...
	if (visualize) {
		...
		// if (clicking) {
		// 	bm.addEventListener(MouseEvent.MOUSE_DOWN, RemoveBrick);
		// 	bm.id = parseInt(userData.slice(6));
		// }
...
function removeBrick(noiseType, noiseLevel, brickFric, fricVar) {
	...
	var noiseGauss = gaussian(0, noiseLevel);
	...
	if (noiseType === "dxy-local") {
		...
		theBrick.ApplyTorque(noiseGauss());
		settle(10);
	} else if (noiseType === "torque-local") {
		var contact_list = getContacts(theBrick);
		contact_list.forEach(function (contact) {
			contact.ApplyTorque(noiseGauss());
		})
